...
app.post('/', (req: express.Request, res: express.Response) => {
  ...
  let cache: string = req.body.cache;
  let cache_port: string = req.body.cachePort;
  let cache_key: string = req.body.cacheKey;
  let scene_uuid: string = req.body.uuid;
  ...
  const redisClient = redis.createClient(Number(cache_port), cache, { auth_pass: cache_key, tls: cache });
  ...
  new Promise((resolve, reject) => {
    redisClient.get(scene_uuid, (err: redis.RedisError | null, result: string) => {
      if (err) {
        reject(err);
      } else {
        resolve(result);
  ...
  .then((scene_information: any) => {
    ...
    return new Promise((resolve, reject) => {
      PT.Camera.render.async(camera, 
                             ...
                             scene_information.image.samples_per_pixel, (err: any, result: any) => {
